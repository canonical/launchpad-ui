...
describe("Button SSR", () => {
  ...
  describe("appearance", () => {
    it("renders with default appearance class by default", () => {
      const { body } = render(Component);
      expect(body).toContain("appearance-default");
    });
    ...
    it.each(appearances)("renders with %s appearance class", (appearance) => {
      const { body } = render(Component, { props: { appearance } });
      expect(body).toContain(`appearance-${appearance}`);
    });
  });

  describe("size", () => {
    it("renders with default size class by default", () => {
      const { body } = render(Component);
      expect(body).toContain("size-default");
    });

    const sizes = ["default", "dense", "small"] as const;

    it.each(sizes)("renders with %s size class", (size) => {
      const { body } = render(Component, { props: { size } });
      expect(body).toContain(`size-${size}`);
    });
  });
  ...
  describe("interactions", () => {
    ...
    it("renders disabled when props is set", () => {
      ...
      expect(body).toContain("visually-disabled");
    });
  ...
  describe("content", () => {
    ...
    it("renders spinner when loading", () => {
      const { body } = render(Component, { props: { loading: true } });
      // TODO: Replace with actual spinner markup if needed
